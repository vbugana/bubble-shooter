...
function circlesColliding(x1, y1, radius1, x2, y2, radius2) {
  //compare the distance to combined radius
  var dx = x2 - x1;
  var dy = y2 - y1;
...
function Grid(rows, cols, bw, bh) {
  ...
  for (var i = 0; i < rows; i++) {
    var r = [];
    for (var j = 0; j < cols - (i - Math.floor(i / 2) * 2); j++) r.push(null);
    this.slots.push(r);
  }
...
Grid.prototype.getCoordForPos = function (i, j) {
  return {
    x: j * this.bw + ((i - Math.floor(i / 2) * 2) * this.bw) / 2,
    y: i * this.bw + this.baseY,
  };
};
...
Grid.prototype.getCellIndexForCoord = function (x, y) {
  ...
  var col = Math.floor(
    (x - (this.bw / 2) * (row - 2 * Math.floor(row / 2))) / this.bw
  );
  //if();
...
Grid.prototype.hasBubbleUpLeft = function (i, j) {
  ...
  return jj >= 0 && jj < this.slots[i - 1].length
    ? this.slots[i - 1][jj] != null
    : false;
};
...
Grid.prototype.addBubble = function (bubble, i, j) {
  if (i <= this.rows && j <= this.cols) {
    //console.log("position where  is: " + bubble.p.x + ", " + bubble.p.y);
    if (this.slots[i][j] == null) this.slots[i][j] = bubble;
    else
      throw {
        error: "El slot no estava vacio",
        oldBubble: this.slots[i][j],
        newBubble: bubble,
      };
...
Grid.prototype.getBubbleGridHeight = function () {
  ...
  for (var i = 0; i < this.slots.length; i++)
    for (var j = 0; j < this.slots[0].length; j++)
      if (this.slots[i][j]) if (i > maxi) maxi = i;
...
Grid.prototype.getAdjacentBubbles = function (i, j) {
  ...
  if (this.hasBubbleDownRight(i, j))
    bubbles.push(this.getBubbleDownRight(i, j));
  return bubbles;
};
...
Grid.prototype.isBubbleOrphan = function (i, j) {
  ...
  if (this.hasBubbleUpRight(i, j)) return this.isBubbleOrphan(uR.i, uR.j);
  if (this.hasBubbleUpLeft(i, j)) return this.isBubbleOrphan(uL.i, uL.j);
  return true;
};
...
Grid.prototype.detachOrphanBubbles = function () {
  for (var i = 0; i < this.slots.length; i++)
    for (var j = 0; j < this.slots[i].length; j++)
      if (this.slots[i][j]) this.slots[i][j].setOrphan(true);

  for (var i = 0; i < this.slots[0].length; i++)
    if (this.slots[0][i]) this.markBubble2(0, i);
...
Grid.prototype.step = function (dt) {
  switch (this.state) {
    case this.states.MOVING_DOWN:
      ...
      for (var i = 0; i < this.slots.length; i++)
        for (var j = 0; j < this.slots[i].length; j++)
          if (this.slots[i][j] != null) this.slots[i][j].p.y += stepSize * dt;
    ...
    case this.states.SHAKING:
      ...
      if (this.shakeRot > Math.PI * 2) this.shakeRot -= Math.PI * 2;
    default:
  }
};
...
TextSprite.prototype.step = function (dt) {
  if (this.active) {
    ...
    this.x += this.vx * dt;
    this.y += this.vy * dt;
    if (this.time > this.delay) this.active = false;
  }
};
...
function PointsTextManager() {
  ...
  this.clearInactive = function () {
    this.texts = this.texts.filter(function (v) {
      return v.active;
    });
  };
  this.step = function (dt) {
    if (this.texts.length > 0) {
      for (var i = 0; i < this.texts.length; i++) this.texts[i].step(dt);
      this.clearInactive();
    }
  };
}
...
var Renderer = (function () {
  ...
  return {
    ...
    drawWorld: function (game, dc, dc2) {
      ...
      //distribution of bubbles in the table
      var dx =
        world.bubblegrid.state == Grid.prototype.states.SHAKING
          ? (Math.sin(world.bubblegrid.shakeRot) * world.bw) / 6
          : 0;
      dc.save();
      dc.translate(dx, 0);
      for (var i = 0; i < world.bubblegrid.slots.length; i++)
      ...
      Renderer.drawText(
        "Time: " + Math.floor(game.endGameAlarm.getRemaining()) / 1000,
        10,
        384
      );

      //Drawing floating texts from scores
      Renderer.setTextFont("15px italic arial,sans-serif strong", "rgb(0,0,0)");
      for (var i = 0; i < world.pointTexts.texts.length; i++)
        Renderer.drawText(
          world.pointTexts.texts[i].text,
          world.pointTexts.texts[i].x,
          world.pointTexts.texts[i].y
        );

      //If paused drawing pause message
...
function World(w, h) {
  ...
  this.bubblegrid = new Grid(
    Math.floor(this.h / this.bw),
    Math.floor(this.w / this.bw),
    this.bw,
    this.bh
  );
  this.bubbles = [];
  ...
  this.bubblegrid.setBubblePopUpCallback(function (b) {
    ...
    world.pointTexts.addTextSprite(
      new TextSprite(b.p.x, b.p.y, 0.02, -0.04, "10")
    );
...
World.prototype.createNextBubble = function () {
  ...
  if (values.length > 0) b.value = values[index];
  return b;
};
...
World.prototype.setup = function () {
  ...
  for (var j = 0; j < bubblesRows; j++)
    for (var i = 0; i < bubblesCols - (j - Math.floor(j / 2) * 2); i++) {
      var b = new Bubble(
        (this.bw / 2) * (j - Math.floor(j / 2) * 2) + i * this.bw + this.bw / 2,
        j * this.bh + this.bh / 2,
        this.bw / 2
      );
      b.setActive(false);
...
World.prototype.testCollision = function () {
  //if there is a shot  verify where it is going
  for (var i = 0; i < this.firedBubbles.length; i++) {
    ...
    if (collides || b.p.y < this.bh + this.bubblegrid.baseY) {
      ...
      //prevent it from being added to a non-enabled cell
      if (pos == null)
        pos = this.bubblegrid.getCellIndexForCoord(b.p.x + b.r, b.p.y);
...
var init = function () {
  ...
  // canvas for  render game
  var c = document.getElementById("c");
  dc = c.getContext("2d");

  // canvas for reading UI
